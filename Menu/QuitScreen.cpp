...
QuitScreen::QuitScreen(SDL_Renderer* gRenderer, SDL_Window* gWindow, SDL_Surface* gScreenSurface, int width, int height):Screen(gRenderer, gWindow, gScreenSurface, width, height)
{
    bgTexture = NULL;
...
bool QuitScreen::LoadTextures()
{
	...
	mBackgroundSurface = loadSurface("graphics/QuitScreen/background.png");
    ...
    mFont = TTF_OpenFont( "graphics/GameScreen/LifeCraft_Font.ttf", 72 );
	if( mFont == NULL )
	{
	...
	if (!GameOverText.loadFromRenderedText("GAME OVER!", {255, 25, 65, 255}, mFont, mRenderer))
	{
...
bool QuitScreen::LoadAudio()
{
   ...
   // Mix_PlayChannel(-1, ButtonClickSound, 10);
    if (success)
...
void QuitScreen::ScreenHandler()
{
    ...
    Button trybutton("TRYAGAIN", &FontTextureSheet, &ButtonTextureSheet, ButtonTextureClips, charRect, FontRowSize, 25, SCREEN_WIDTH/2-200, SCREEN_HEIGHT/2+75+90, 400, 60);
    ...
    while (!quit)
    {
        while( SDL_PollEvent( &e ) != 0 )
        {
            ...
            if( e.type == SDL_MOUSEMOTION || e.type == SDL_MOUSEBUTTONDOWN || e.type == SDL_MOUSEBUTTONUP )
            {
                ...
                if (trybutton.Animation(&e, x, y, ButtonClickSound)) {   quit = true; closingstatus = Retry;  }
        ...
        trybutton.render(mRenderer);
        quitbutton.render(mRenderer);
        volume.render(mRenderer);
        music.render(mRenderer);
        GameOverText.render(450, 300, mRenderer, NULL, 700, 100);
...
int QuitScreen::getClosingStatus()
{
